...
public:
	...
	BinTree<int> reconstruir(
		std::vector<int> const &preorden, std::vector<int> const &inorden)
	{
		
		if(preorden.size() == 1){
			return BinTree<int>(preorden[0]);
		}
		else if(preorden.empty()){
			return BinTree<int>();
		}
		
		int rootval = preorden[0];

		int j = 0;
	
		while( j < inorden.size() && inorden[j] != rootval){
			j++;
		}
	
		
		std::vector<int>  pre_i(preorden.begin()+1, preorden.begin()+j+1);
		std::vector<int>  pre_d(preorden.begin()+j+1, preorden.end());
		std::vector<int> in_i(inorden.begin(),inorden.begin()+j);
		std::vector<int> in_d(inorden.begin()+j+1,inorden.end());

		
		BinTree<int> izq = reconstruir(pre_i,in_i);
		BinTree<int> der = reconstruir(pre_d,in_d);



		return BinTree<int>(izq,rootval,der);
	}
