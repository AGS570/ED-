...
public:
  ...
  {
    return inter(this->root_node,lower,upper);
  }
...
private:
  ...
  static int inter(Node *rt, const T &lower, const T &upper)
  {

    if(rt == nullptr) return 0;
    if(rt->elem < lower) return inter(rt->right,lower,upper);
    if(rt-> elem  > upper) return inter(rt->left,lower,upper);

    return 1 + inter(rt->left,lower,upper) +inter(rt->right,lower,upper);
  }
